  ...
  if (rejecting) {
    return (
      <div style={{ marginTop: 14 }}>
        <textarea
          style={{ width: '100%', background: 'var(--page-bg)', border: '1px solid var(--border)', borderRadius: 10, padding: 10, fontSize: '0.84rem' }}
          rows={2}
  ...
  function openDetail(item) {
    modal.open({
      ...
      content: (
        <>
          {item.screenshot_url ? (
            <img src={item.screenshot_url} style={{ width: '100%', borderRadius: 12, marginBottom: 14 }} />
          ) : (
      ...
      <div className="table-wrap">
        <div className="table-head">
          <div className="filter-row">
            {FILTERS.map((s) => (
              <span key={s} className={`filter-chip ${filter === s ? 'on' : ''}`} onClick={() => setFilter(s)}>
                {s[0].toUpperCase() + s.slice(1)}
              </span>
            ))}
          </div>
          <div className="tb-search pay-search">
            <span>🔍</span>
            <input placeholder="UTR, Order ID, Name..." onChange={(e) => onSearchChange(e.target.value)} />
          </div>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table>
            ...
            <tbody>
              {loadingList ? (
                <tr><td colSpan={7}><div className="skel" style={{ height: 20 }} /></td></tr>
              ) : list.length === 0 ? (
